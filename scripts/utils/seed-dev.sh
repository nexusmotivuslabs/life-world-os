...
# This script seeds the development database with initial data
# Note: Seeding is NOT run automatically on container startup
# Run this script when:
#   - Setting up a new development environment
#   - After resetting the database (docker-compose down -v)
...
echo -e "${BLUE}=== Life World OS - Seed Development Database ===${NC}\n"
...
# Check if dev backend container is running
if ! docker ps --format '{{.Names}}' | grep -q "life-world-os-backend-dev"; then
    echo -e "${YELLOW}⚠️  Development backend container is not running${NC}"
...
CONTAINER_NAME="life-world-os-backend-dev"
