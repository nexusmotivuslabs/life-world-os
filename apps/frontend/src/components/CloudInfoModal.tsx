...
import { X, TrendingUp, TrendingDown, Minus, Link2, Lightbulb } from 'lucide-react'
import { getCloudInfo, getCloudStrengthStatus, getCloudEffects, CloudType } from '../lib/cloudInfo'

interface CloudInfoModalProps {
  type: CloudType | null
  strength: number
  isOpen: boolean
  onClose: () => void
...
export default function CloudInfoModal({ type, strength, isOpen, onClose }: CloudInfoModalProps) {
  ...
  const effects = getCloudEffects(cloudInfo, strength)
            ...
            >
              ...
              <div className="sticky top-0 bg-gray-800 border-b border-gray-700 p-6">
                <div className="flex items-start justify-between mb-4">
                  <div className="flex items-start gap-4 flex-1">
                    <div className="text-4xl">{cloudInfo.icon}</div>
                    <div className="flex-1">
                      <h2 id="cloud-modal-title" className="text-2xl font-bold text-white mb-2">
                        {cloudInfo.name}
                      </h2>
                    </div>
                  </div>
              ...
              <div className="p-6 space-y-6">
                ...
                <div>
                  ...
                  <p className="text-gray-300 leading-relaxed">{cloudInfo.fullDescription}</p>
                </div>
                ...
                {/* System Relationships - Highlighted Key Stats */}
                <div className={`${config.bgColor} ${config.borderColor} border-2 rounded-lg p-4`}>
                  <h3 className="text-lg font-semibold text-white mb-4 flex items-center gap-2">
                    <Link2 className={`w-5 h-5 ${config.color}`} />
                    Key Stats & System Relationships
                  </h3>
                  <div className="space-y-4">
                    {cloudInfo.affectsResources.length > 0 && (
                      <div className="bg-gray-900/50 rounded-lg p-3">
                        ...
                        <ul className="space-y-1.5">
                          {cloudInfo.affectsResources.map((resource, index) => (
                    ...
                    {cloudInfo.affectsXP.length > 0 && (
                      <div className="bg-gray-900/50 rounded-lg p-3">
                        <p className="text-sm font-semibold text-purple-400 mb-2 flex items-center gap-2">
                          ...
                          Affects XP Categories:
                        </p>
                        <ul className="space-y-1.5">
                          {cloudInfo.affectsXP.map((xp, index) => (
                            <li key={index} className="text-gray-200 text-sm flex items-start gap-2">
                              <span className={`${config.color} mt-1`}>•</span>
                              <span>{xp}</span>
                            </li>
                    ...
                    {cloudInfo.relatedSystems.length > 0 && (
                      <div className="bg-gray-900/50 rounded-lg p-3">
                        ...
                        <ul className="space-y-1.5">
                          {cloudInfo.relatedSystems.map((system, index) => (
                            <li key={index} className="text-gray-200 text-sm flex items-start gap-2">
                              <span className={`${config.color} mt-1`}>•</span>
                              <span>{system}</span>
                            </li>
                ...
                <div>
                  ...
                  <ul className="space-y-2">
                    {cloudInfo.improvementTips.map((tip, index) => (
                      <li key={index} className="flex items-start gap-2 text-gray-300">
